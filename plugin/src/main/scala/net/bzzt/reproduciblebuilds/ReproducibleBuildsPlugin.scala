...
import sbt.{io => _, _}
import sbtcompat.PluginCompat.{FileRef, toFile, toFileRef}
import spray.json._
...
object NameHelper {
  def nameOf(ref: FileRef) = ref.name
}
...
object ReproducibleBuildsPlugin extends AutoPlugin {
  ...
  lazy val ourCertification = Def.task[Certification] {
    ...
    val artifacts = (Compile / packagedArtifacts).value
    Certification(
      organization.value,
      reproducibleBuildsPackageName.value,
      version.value,
      scmInfo.value,
      artifacts.mapValues(toFile).toMap,
  ...
  private def reproducibleBuildsCheckImpl(resolver: Resolver, info: SubstituteInfo) = Def.task {
    ...
    val ourArtifactFilesByFilename = ourArtifacts.map { case (_, file) => (nameOf(file), file) }.toMap
    val publishedUrlByFilename = (Compile / packagedArtifacts)
      .map(m => m.keys.map(a => (nameOf(m(a)), artifactUrl(resolver, a.extension, a.classifier, info))))
      .value
      .toMap
